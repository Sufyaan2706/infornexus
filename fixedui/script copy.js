...
function renderTables(orderData) {
    ...
    Object.keys(groups).forEach(lineAgg => {
        ...
        extractBtn.className = 'pack-btn';
        extractBtn.style.marginLeft = '10px';
        extractBtn.innerText = 'Extract to JSON';
        lineAggSection.appendChild(extractBtn);

        extractBtn.onclick = () => {
            extractOrderDataToJson(orderData);
            alert("JSON extracted! Open your browser's Developer Tools (F12) Console to view it.");
        };
        // Container to hold tables side-by-side
        const tablesContainer = document.createElement('div');
        tablesContainer.className = 'tables-container';

        // Left side: Original table
        ...
        // Right side: Packing table target container
        ...
        <table class="packing-table">
            <thead>
                ...
                <td colspan="1" class="input-label">Input weight</td>
                    ${sizes.map(s => `
                        <td>
                        <input type="number" class="max-qty-header-input" min="0">
                        </td>
                    `).join('')}
                </tr>
            </thead>
...
function extractOrderDataToJson(orderData) {
    ...
    // Step 1: Group by Line Aggregator and Size, and collect item arrays
    orderData.orderItem.forEach((item) => {
        const bi = item.baseItem || {};
        const ref = bi.reference || {};
        const id = bi.itemIdentifier || {};

        /* Note: Variance paths vary by API payload. I am using a generic 
           fallback (variance.upperBound). You may need to tweak this path
           if it is named differently in the actual Infor Nexus response. */
        const variance = bi.variance || {};

        const lineAgg = ref.LineAggregator ?? 'Unknown';
        const size = id.IdBuyerSize ?? 'Unknown';

        // Initialize LineAgg level if it doesn't exist
        if (!jsonSchema[lineAgg]) {
            jsonSchema[lineAgg] = {};
        }

        // Initialize Size level if it doesn't exist
        if (!jsonSchema[lineAgg][size]) {
            jsonSchema[lineAgg][size] = {
                maxBoxQty: 50, // Hardcoded default based on your packing logic
                cartonNumbers: "",
                weightPerItem: 0, // Assuming this isn't in standard payload, defaulting to 0
        ...
        // Build the specific item detail object based on your handwritten schema
        const itemDetail = {
            ...
            quantity: qty,
            variance: {
                ub: parseFloat(variance.upperBound || variance.UB || 0),
                lb: parseFloat(variance.lowerBound || variance.LB || 0)
            }
        };

        // Push the item into the list and update the running total for that size
        jsonSchema[lineAgg][size].items.push(itemDetail);
        jsonSchema[lineAgg][size].totalShipmentQtyPerSize += qty;
    });

    // Step 2: Calculate Carton Numbers for each LineAgg -> Size
    for (const lineAgg in jsonSchema) {
        let cartonNo = 1; // Resets carton numbering for each new Line Aggregator

        for (const size in jsonSchema[lineAgg]) {
            const sizeData = jsonSchema[lineAgg][size];
            const totalQty = sizeData.totalShipmentQtyPerSize;
            const maxQty = sizeData.maxBoxQty;

            const fullCartons = Math.floor(totalQty / maxQty);
            const remainder = totalQty % maxQty;

            let startCtn = cartonNo;
            let endCtn = cartonNo;

            // Calculate the string for full cartons
            if (fullCartons > 0) {
                endCtn = cartonNo + fullCartons - 1;
                cartonNo = endCtn + 1;
            }
            // Add a carton if there is a remainder
            if (remainder > 0) {
                endCtn = cartonNo;
                cartonNo++;
            }

            // Assign the formatting (e.g., "1" or "1-3")
            if (totalQty > 0) {
                sizeData.cartonNumbers = startCtn === endCtn ? `${startCtn}` : `${startCtn}-${endCtn}`;
            } else {
                sizeData.cartonNumbers = "N/A";
            }
        }
    }
